...
function pingDevice(event) {
    ...
    fetch('/api/v2/ping.php?antip=' + routerIP)
        .then(response => response.json())
        .then(data => {
            // Create and display ping result element
            const pingResultElement = createPingResultElement(data.antenna.ip, data.antenna.status, data.antenna.response_time);
    ...
    if (smIP) {
        fetch('/api/v2/ping.php?wanip=' + smIP)
            .then(response => response.json())
            .then(data => {
                // Create and display ping result element
                const pingResultElement = createPingResultElement(data.router.ip, data.router.status, data.router.response_time);
                dialog.appendChild(pingResultElement);
            })
            .catch(error => {
                console.error('Error:', error);
